...
Proof.
 intros. induction l.
 - simpl. reflexivity.
 - simpl. rewrite <- IHl. rewrite rev_app_distr. rewrite IHl. rewrite IHl. simpl. reflexivity.
Qed.
...
Example test_filter2: filter length_is_1
...
Definition filter_even_gt7 (l : list nat) : list nat :=
 filter (fun x => (even x) && (negb(x <? 7))) l.
...
Proof.
 intros. induction l.
 - simpl. reflexivity.
 - simpl. rewrite <- IHl. rewrite map_dist. simpl. reflexivity.
Qed.
...
The definitions and uses of filter and map use implicit arguments in many places. 
Replace the curly braces around the implicit arguments with parentheses, 
and then fill in explicit type parameters where necessary and use Coq to check that you've done so correctly
...
Theorem nth_error_informal : forall X l n, length l = n -> @nth_error X l n = None.
Proof.
 intros. induction l.
 - reflexivity.
 - (*since any array length N contains N-1 element (numbersing of element start with zero)
...
app_nil_r : forall (X:Type), forall l:list X, l ++ [] = l.
app_assoc : forall A (l m n:list A), l ++ m ++ n = (l ++ m) ++ n.
app_length : forall (X:Type) (l1 l2 : list X), length (l1 ++ l2) = length l1 + length l2.
rev_app_distr: forall X (l1 l2 : list X), rev (l1 ++ l2) = rev l2 ++ rev l1.
rev_involutive : forall X : Type, forall l : list X, rev (rev l) = l.
map_dist : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X), map f (l1 ++ l2) = map f l1 ++ map f l2.
map_rev : forall (X Y : Type) (f : X -> Y) (l : list X), map f (rev l) = rev (map f l).
*)
